...
## 设置、快捷键与权限
- 首次启动时，若缺少 `辅助功能` 授权，应用会先进入独立的权限引导窗口；完成后才进入正常使用态
- 辅助功能授权入口接入 [PermissionFlow](https://github.com/jaywcjlove/PermissionFlow)，会自动打开系统设置并提供拖拽引导浮层
- 设置窗口是独立窗口，只展示 `global shortcut`、状态栏菜单内 `⌘1` 到 `⌘3` 的模式切换职责、权限状态与重新检查入口
- 全局快捷键注册已改为使用 [HotKey](https://github.com/soffes/HotKey)；设置页录制仍由应用自身维护
...
- 若 `clipboard fallback` 成功，应用会尽量恢复用户原有剪贴板；但如果此期间用户或其他程序已更新剪贴板，则不会强制覆盖新内容
- 首启强制引导只覆盖 `辅助功能`；`提醒事项` 继续按需单独授权
- 若当前系统对仅含 `⌥ / ⇧` 的全局快捷键支持受限，设置页需要明确提示用户改为包含 `⌘` 或 `⌃` 的组合
- 权限提示需要明确区分辅助功能相关能力与提醒事项授权，不能让失败表现为静默无响应
