...
## 设置、快捷键与权限
- 首次启动时，若缺少 `辅助功能` 权限，应用先进入独立权限引导窗口；完成前不进入正常使用态
- 辅助功能授权入口接入 [PermissionFlow](https://github.com/jaywcjlove/PermissionFlow)，会自动打开系统设置并提供拖拽引导浮层
- 设置窗口只负责“快捷键与权限”，不承载工具功能
- 全局快捷键注册通过 [HotKey](https://github.com/soffes/HotKey) 管理；设置页录制与持久化继续由应用自身负责
- 状态栏菜单只负责切换默认模式和打开设置
...
- 首启强制引导只覆盖 `辅助功能`；`提醒事项` 继续保留按需授权
- 若当前系统对仅含 `⌥ / ⇧` 的全局快捷键支持受限，设置页需要提示用户改为包含 `⌘` 或 `⌃` 的组合
- 权限提示必须清楚区分辅助功能相关能力与提醒事项授权，不能把失败表现成静默无响应
