...
- `Date String`：直接展示时间戳
- `Plain Text`：不自动生成结果，只展示可执行动作，并将 `MD5` 作为优先推荐动作
...
`创建提醒事项` 功能实现保留，但在当前阶段不作为状态栏菜单默认模式暴露，也不提供对应模式切换快捷键。
自动识别已可稳定命中的 `JSON`、时间戳和日期字符串不再单独暴露顶部默认模式入口。

### 5.5 Option 型二次操作规则
当某个能力存在“基于同一输入对当前结果做可逆切换”的 option 时，统一作为底部 `secondary action` 暴露，而不是额外放在输入区或顶部区域。

统一交互规则如下：
- 底部动作顺序保持为：`复制结果`、`替换原文`、option 型 `secondary action`
- option 按钮始终显示“下一步动作”，不直接显示当前状态
- 点击后必须基于同一输入重新生成 `primary result`
- 结果切换后，按钮文案同步翻转到相反动作

当前 v1 已纳入该规则的能力：
- `MD5`：默认输出小写，按钮在 `转大写` 与 `转小写` 之间切换
- `日期转时间戳`：默认输出秒级，按钮在 `转毫秒` 与 `转秒级` 之间切换

当前不纳入该规则的能力：
- `时间戳转本地时间`：按输入的 `10` 位或 `13` 位时间戳自动识别，不单独暴露秒级 / 毫秒级切换按钮

后续新增任何带二次切换或附带 option 的功能，都应复用本节规则，避免为单一能力重新发明交互。
...
其中底部动作栏需要长期保持以下约束：
- `复制结果` 与 `替换原文` 作为基础动作优先出现
- option 型二次操作始终跟在基础动作之后
- 同一能力在 `result panel` 与独立工具窗口中的 option 交互必须一致
- 当处于 `Plain Text` 的 `actionsOnly` 状态时，不展示空的复制/替换基础动作，占位由推荐动作替代，首位动作为 `MD5`
- 只有当前结果持有可安全写回的原始目标时，才展示 `Replace Selection`
- `clipboard fallback` 结果默认不展示 `Replace Selection`，避免把基于旧剪贴板的结果误写到当前焦点
...
- `clipboard fallback` 只用于继续计算和展示结果，不应暗示可以安全回写到当前实时选区
- 若回退成功，应用应尽量恢复用户原有剪贴板内容；但若用户或其他程序已在此期间更新剪贴板，则不应强制覆盖该新内容
